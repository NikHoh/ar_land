#include "ar_land/track_trajectory_planner_node.hpp"
#include <tf2/transform_datatypes.h>
#include <angles/angles.h>
#include "ar_land/tools.hpp"

track_trajectory_planner_node::track_trajectory_planner_node()
  : flight_state(Idle)
  , thrust(0)

{
  ROS_INFO("Im Konstruktor des track_trjactory_planners");
  // initialize topics
  ros::NodeHandle n("~");
  ...
  // Subscribers
  T_cam_board_sub = nh.subscribe(T_cam_board_topic, 1, &track_trajectory_planner_node::setGoalinWorld, this); // subscribed zu (1) und führt bei empfangener Nachricht (3) damit aus
  control_out_sub = nh.subscribe("cmd_vel",1, &track_trajectory_planner_node::getValue, this);
  ...
  //Services
  flight_state_change_srv = nh.advertiseService("flight_state_change", &track_trajectory_planner_node::state_change, this);

}

bool track_trajectory_planner_node::state_change(ar_land::flight_state_changeRequest &req,
...
void track_trajectory_planner_node::getValue(const geometry_msgs::Twist &msg){
  last_thrust = msg.linear.z;
}


void track_trajectory_planner_node::setGoalinWorld(const geometry_msgs::TransformStamped &T_cam_board_msg) {
...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, "track_trajectory_planner_node"); // initializes node named track_trajectory_planner_node

  ros::NodeHandle n("~");
  track_trajectory_planner_node node;                // Creates trajectory_planner_node
  ros::spin();
